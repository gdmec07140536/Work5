package s07140536.gdmec.cn.work5;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        player.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
            ...
            public void onPrepared(MediaPlayer mp) {
                ...
                vHeigth = player.getVideoHeight();
                if (vWidth > currDisplay.getWidth() || vHeigth > currDisplay.getHeight()) {
                    float wRatio = (float) vWidth / (float) currDisplay.getWidth();
                    float hRatio = (float) vHeigth / (float) currDisplay.getHeight();
                    float ratio = Math.max(wRatio, hRatio);
                    vWidth = (int) Math.ceil((float) vWidth / ratio);
                    vHeigth = (int) Math.ceil((float) vHeigth / ratio);
                    surfaceView.setLayoutParams(new LinearLayout.LayoutParams(vWidth, vHeigth));
                    player.start();
                } else {
                    player.start();
                }
